...
fn insert_get_remove_one() {
    ...
    assert_eq!(slab[Rc::clone(&key)], 10);
    assert_eq!(slab.get(Rc::clone(&key)), Some((&10, Rc::clone(&key))));
    assert!(!slab.is_empty());
    assert!(slab.contains(*key));

    assert_eq!(slab.remove(Rc::clone(&key)), 10);
    assert!(!slab.contains(*key));
...
fn slab_get_mut() {
    ...
    slab[Rc::clone(&key)] = 2;
    assert_eq!(slab[Rc::clone(&key)], 2);

    *slab.get_mut(Rc::clone(&key)).unwrap().0 = 3;
...
fn key_of_layout_optimizable() {
    ...
    slab.insert("quux");
    assert_eq!(slab.key_of(&slab[Rc::clone(&third)]), third);
}
...
fn key_of_zst() {
    ...
    slab.insert(());
    assert_eq!(slab.key_of(&slab[Rc::clone(&second)]), second);
}
...
fn into_iter() {
    ...
    slab.remove(Rc::new(0));
    slab.remove(Rc::new(4));
    slab.remove(Rc::new(5));
    slab.remove(Rc::new(7));
...
fn try_remove() {
    ...
    assert_eq!(slab.try_remove(Rc::clone(&key)), Some(1));
    assert_eq!(slab.try_remove(Rc::clone(&key)), None);
